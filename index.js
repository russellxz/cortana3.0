...
if (global.db.READ) return new Promise((resolve) => const interval = setInterval(function () { (!global.db.READ ? (clearInterval(this), resolve(global.db.data == null ? global.loadDatabase() : global.db.data)) : null) }, 1 * 1000))
...
if (global.db) const interval = setInterval(async () => {
...
const delay = (ms) => new Promise((resolve) => const timeout = setTimeout(resolve, ms))
...
const timeout = setTimeout(printRainbowMessage, 60000) //Ajuste el tiempo de espera a la velocidad deseada
}
